                ...
                try
                {
                    ...
                    foreach (var w in dialogs)
                    {
                        ...
                        else if (w is Dialog_GiveName giveName)
                        {
                            ...
                            catch (Exception ex) { McpLog.Warn($"[GetOpenDialogs] 翻译弹框文本失败: {ex.Message}"); prompt = msgKey; }
                        ...
                        else
                        {
                            ...
                            sb.AppendLine();
                            if (canClose)
                                sb.AppendLine($"## 弹框 [{dialogIdx}] {typeName}\n[0] 关闭");
                            else
                                sb.AppendLine($"## 弹框 [{dialogIdx}] {typeName} (无法关闭，需手动处理)");
                            dialogIdx++;
                        }
                    }
