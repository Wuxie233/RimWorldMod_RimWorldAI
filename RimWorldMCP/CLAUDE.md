...
**第 1 层 — 事件分级响应**：有新事件时按级别分流处理。事件按 `GetEventLevel()` 分为 4 级（L0~L3），L3 Critical 暂停游戏，L1/L2 注入工具返回值。

```
任何待推送事件
  → Drain 通知队列
  → GetEventLevel() 分级
  → L3 Critical: AI 忙时暂停游戏 + DangerSummary 注入
  → L2/L1: 累加到 _pendingLevel12Count，发聊天
  → L0 Silent: 丢弃（不计数、不发聊天、不注入）
  → ToolRegistry 在每个工具返回末尾注入: "📋 新事件: N件 | 如需处理请用 toggle_pause 暂停"
```

**事件分级表**:

| 级别 | 聊天 | 工具结果 | 暂停 | Letter | Message | Alert |
|------|------|---------|------|--------|---------|-------|
...
6 个 Harmony Patch 拦截游戏事件 → `NotificationBus` → `GetEventLevel()` 分级。
...
# 游戏设置：自动启动 = 关闭，远程连接主机 = 127.0.0.1，远程连接端口 = 19999
```
...
| `UI/Dialog_AiChat.cs` | 顶部横幅（警告/超限）+ 底栏进度条 |
| `RimWorldMCPMod.cs` | 设置窗口：预算配置 + 全局用量汇总表 + 清空按钮 |
| `Bridge/CCClient.cs` | 从 SDK 消息提取模型名，hello 附加 budget 字段 |
| `cc-companion/companion/config.ts` | `tokenBudgetLimit/Used/Action` 类型 |
| `cc-companion/companion/companion.ts` | 解析 hello budget，辅助 enforcement |
| `cc-companion/bridge/session.ts` | 传递模型名到 usage 数据 |
