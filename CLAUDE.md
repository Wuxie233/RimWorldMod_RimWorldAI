...
| `designate_harvest` | 标记作物收割（仅已成熟的 Standard 作物，支持矩形范围） | `DesignationManager.AddDesignation(HarvestPlant)` (入队) |
| `designate_deconstruct` | 标记建筑拆除（逐格查找最上层建筑，支持矩形范围） | `DesignationManager.AddDesignation(Deconstruct)` (入队) |
