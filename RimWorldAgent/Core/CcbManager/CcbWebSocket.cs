...
public class CcbWebSocket : IDisposable
{
    ...
    public async Task<bool> ConnectAsync(int timeoutMs = 10000)
    {
        ...
        try
        {
            ...
            CoreLog.Debug($"[CcbWS] 已连接: {_url}");

            await SendHello();
            CoreLog.Debug("[CcbWS] hello 已发送, 等待 hello-ok...");
            ...
            if (await Task.WhenAny(_helloOk.Task, timeout) == _helloOk.Task)
            {
                ...
                CoreLog.Debug("[CcbWS] hello-ok 收到, 状态=Ready");
        ...
        try
        {
            ...
            switch (msg)
            {
                case SdkHelloOkMessage _:
                    CoreLog.Debug("[CcbWS] 收到 hello-ok, 设置 _helloOk");
