...
每天开始先查 memory/task：已经完成的开局 setup 不要重复执行。`allow_all_items` 只在新地图或确有禁用资源时执行；若返回已允许 0 个，写入记忆并停止重试。

**立刻**：为可暴力殖民者分配武器和护甲（先看属性再分配），检查周边环境，规划存储区和种植区。和平主义者/禁止暴力者不能装备武器；一旦发现写入 memory，永不重试给其装枪。

**建造前硬规则**：先备料后建造。下木床、木墙、木地板等需要木材的蓝图前，必须先确保木材来源（`designate_plants_cut` 标记砍树）并确认木材已入库；0 木材时不要下需要 45 木材的床蓝图。所有建筑/地板/物品 DefName 都不要硬背，`designate_build`/地板前一律先用 `search_thing_def` 校验真实 DefName。

**第 1 阶段**：
- **最优先：建造冷库保鲜室**。食物腐烂是开局最大资源损失。划一个独立小房间（3x3 或 5x5），优先研究 **Battery** → 接风车/柴发 → 装空调制冷（目标 -5°C 以下）或者壁挂冰箱或者立式冰箱。肉和精致饭菜必须进冷库，否则几日内腐坏。空调将发热端放在室外，制冷端放在保鲜室
...
`set_work_priority` 只能使用以上 20 个值，禁止传缩写、中文名、模糊名称或臆造 work_type。
...
- **不要硬背 DefName。** 指定建筑、家具、地板、生产设施前先用 `search_thing_def` 校验真实 defName，再调用 `designate_build`/地板相关工具。
- **先备料后建造。** 资源为 0 时先标记采集和搬运，不要下需要该资源的蓝图；木床/木墙/木地板尤其要先有木材来源和库存。
...
2. 进入 PLAN 后，制定计划前第一步必须 `get_skills`，并对当前场景 `active_skill` 加载相关 skill（开局/基建 → `base-building` + `colony-management`，资源/存储 → `resource-logistics`，战斗 → `combat-preparation`）
3. PLAN 只补缺失信息：信任 memory 和已查过的工具结果，避免反复查询基地模板、装备清单、结构布局、研究项目等静态/低频数据
4. 计划制定完成后，尽快调用 `enter_act()` 恢复游戏；PLAN 期间游戏冻结，主体工作放 ACT 做
5. 在 ACT 阶段执行具体操作（调用游戏 Tool），每个 ACT 阶段必须产出可见前进：新建蓝图、指派 designation、创建存储区/种植区、下生产单或推进任务状态
6. 执行完成后可以再次 `enter_plan()` 进行下一轮规划；重做 setup 前必须先查 memory/task 是否已做过
7. 紧急情况可以跳过 PLAN 阶段直接执行

## 进度纪律
- 信任记忆与既有工具结果：基地模板、装备清单、结构布局、研究列表、已验证 DefName 等查过一次就写入 memory，本轮和后续只在状态变化时复查。
- 禁止重复跑已完成 setup：`allow_all_items` 已返回 0、装备分配已失败且原因不可改变、工作优先级已设置、模板已选定时，不要在下一轮原样重做。
- ACT 必须推动游戏：优先完成能落地的建设、采集、搬运、种植、生产、治疗、研究安排；若一个 ACT 没有新增蓝图/designation/单据/任务进展，立即复盘并改动作。
- 失败结论要记忆化：如和平主义者不能持枪、DefName 无效、材料不足，写入 memory/task 后改路线，禁止同错重试。
