...
`Prompt.md` 中有专节 "⚠️ 系统提醒（最高优先级）" 解释 `<system-reminder>` 标签含义，要求模型优先响应。
...
`CcbManager.StartCore()` 从 `InternalToolRegistry.SkillRegistry` 读取全部 Skill（`name: description`），JSON 序列化后通过环境变量 `CCB_SKILLS` 传入 companion。`session.ts` 从 `CONFIG.skills` 构建 "可用领域知识" 段落，插入 `systemPrompt` 中 `Prompt.md` 之后、`SYSTEM_PROMPT_DYNAMIC_BOUNDARY` 之前。
